...
updating <- function (name, priors_beta_mu1, priors_beta_sd1, priors_beta_mu0, priors_beta_sd0) {
  ...
  # At this point, we have normalized the data's x, y values and have also scaled down the mu and sd of the prior distribution to suit the needs
  
  calc_x <- function(x) {
  # qnorm - determines the boundary value given the area. qnorm(0.85) gives the 85th percentile of a normal distribution
    exp((qnorm(x) - (-1 / priors_beta_mu0) * log(priors_beta_mu1 / norm_factor)) * priors_beta_mu0)
  }
  # this function calculates the value of x in the distribution(?)
  ...
  get_coeficients <- function(lower, lower_bern, upper, upper_bern) {
    ...
    # alphas are related to priors 0, 1 - their parameters
    a <- list("a0"=as.numeric(priors.alpha[1]), "a1"=as.numeric(priors.alpha[2]))
    return(a)
  }
  # what coefficients are we getting here? - they are getting a0, a1 which are used to find the prior distributions for 0 and 1
  ...
  get_a_mu_sd <- function(
    ...
    list(
      ...
      "sd0"=abs(a_m$a0 / 10),
      "mu1"=a_m$a1,
      "sd1"=abs(a_m$a1 / 10) # does not want to deviate from the mu from the solution a lot
    )
  }
  
  priors.zero <- get_a_mu_sd(0.01, 0.99, 0.05, 0.01) # where did they get the values from?
  ...
  priors.zero.random0 <- rnorm(iterations, priors.zero$mu0, priors.zero$sd0)
  priors.zero.random1 <- rnorm(iterations, priors.zero$mu1, priors.zero$sd1)
  priors.one.random0 <- rnorm(iterations, priors.one$mu0, priors.one$sd0)
  priors.one.random1 <- rnorm(iterations, priors.one$mu1, priors.one$sd1)
  ...
  priors.sample$zero <- bernoulli_f_i(priors.zero.random0, priors.zero.random1, priors.sample$iteration, priors.sample$x_plot_norm)
  priors.sample$one <- bernoulli_f_i(priors.one.random0, priors.one.random1, priors.sample$iteration, priors.sample$x_plot_norm)
  ...
  beta_f <- function(a, b, x) {
    pnorm((1 / a) * log((x * norm_factor) / b)) # pnorm calculates the CDF where x is normal. So this is cumulative lognormal distribution which
  }
  ...
  cat(
    ...
    model{
      # priors
      # dnorm is specified in terms of mean and precision.
      ...
      b0 ~ dnorm(priors.b0_mu, 1 / (priors.b0_sigma ^ 2))  # sigma to tau (tau = 1 / sigma**2)
  ...
  model <- jags.model("beinf.txt", data=jd, n.chains=3, n.adapt=1000)
  
  update(model, 1000)
  
  out <- coda.samples(model, c("zero0", "zero1", "one0", "one1", "b0", "b1", "phi"),
                      n.iter=100000, thin = 100)
  # this is for generating actual samples after burn out
